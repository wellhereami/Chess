...
public class GameWindow extends JPanel implements Runnable{
    ...
    public GameWindow(){ 
        ...
        try {
            InputStream is = getClass().getResourceAsStream("/fonts/AncientMedium.ttf");
    ...
    public void paintUI(Graphics2D g2){
        ...
        g2.setFont(f.deriveFont(20f));
        //Turn counter
        String teamName = currTeam == 1 ? "White" : "Black";
        g2.drawString(teamName + ",", columns * stepSize + 10, 25);
        g2.drawString("it is your turn.", columns * stepSize + 10, 50);

        if(inCheck == 1){
            g2.drawString("White is in check", columns * stepSize + 10, 60);
        }
        if(inCheck == -1){
            g2.drawString("Black is in check", columns * stepSize + 10, 60);
        }

        //Captured Pieces White
        g2.drawString("White", columns * stepSize + 10, height/4 + 50);
        g2.drawLine(columns * stepSize + 10, height/4 + 53, columns * stepSize + 50, height/4 + 53);

        //Captured Pieces Black
        g2.drawString("Black", columns * stepSize + 10, height/2 + 75);
        g2.drawLine(columns * stepSize + 10, height/2 + 78, columns * stepSize + 50, height/2 + 78);
    }
